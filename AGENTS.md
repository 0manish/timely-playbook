...
- Includes `timely-trackers/`, `templates/`, and `snippets/`.
...
- Extends to agent execution behavior when changes are made through
  `tools/orchestrator/` or direct `codex` runs targeting this repository.

## Policy stack for integrated agents

- Governing order for agentic execution is: `AGENTS.md`, then
  `.orchestrator/ownership.yaml`, then `SKILLS.md`, then task-specific asks.
...
- **Primary operator:** {{OWNER_NAME}} (`{{OWNER_EMAIL}}`)
- **Escalation backup:** self; note escalation in the ledger decision log.

## Decision rights & escalation

- Documentation steward listed in `timely-trackers/agent-control-ledger.md`
  approves structural or taxonomy changes.
- Escalate conflicts (e.g., spec vs. implementation discrepancies) via the
  ledger and capture mitigation tasks in `timely-trackers/todo-backlog.md`.
...
- Format Markdown with the workspace standard (`.markdownlint.json` when
  present). Use sentence-case headings and active voice.
...
- Run link validation (`./scripts/check-doc-links.sh`) before merging structural
  updates; log results in `timely-trackers/test-run-journal.md`.
- Manual cadence: re-run `./scripts/check-doc-links.sh` plus
  `./scripts/run-markdownlint.sh` every Sunday retro and capture output in the
  journal.
...
- Update the spec traceability matrix
  (`timely-trackers/spec-traceability.md`) whenever documentation adds or
...
- Follow the checklists in `timely-trackers/ceremony-agendas.md` when preparing
  sync or retro notes.
- Keep the navigation hub (`Timely-Governance-Index.md`), tracker guidance
  (`Timely-Governance-Trackers.md`), and flowgraph (`Timely-Flowgraph.md`)
  current as new assets land.
...
- Keep the Mermaid flowgraph (`Timely-Flowgraph.md`) current by adding nodes and
  edges for every new artifact, template, or automation routine.
- Trigger `./scripts/check-doc-links.sh` via CI
...
- This repository is the base template for orchestrated, Codex-ready projects.
  Seed new repos with `scripts/bootstrap-timely-template.sh`, `timely-playbook seed`,
  or a packaged `dist/timely-template.tgz` export.
...
- VS Code is the operator console (planning, delegation, review). Automation
  lives in `tools/orchestrator/`.
- `.orchestrator/ownership.yaml` defines agent blast radii;
  `.orchestrator/state.json` tracks tasks, CI events, and deployments.
- `.vscode/tasks.json` exposes the orchestrator commands (`plan`, `start-ready`,
  `record-ci`, `update-status`, and FullStack commands) so agents can run them
  deterministically.
- `.github/workflows/ci.yml` keeps validation and orchestrator status in CI;
  `.github/workflows/autofix.yml` runs Codex repairs on CI failures.

## KISS-to-Codex incorporation policy (A-E)
...
- Section E — compatibility control: only features with deterministic CI value
  and reversible changes are allowed in skilled Codex flows.
...
- Required for each skilled Codex run: include success outcome, summary, changed
  files, and rollback hint in final output.
- Fullstack integration must remain command-driven via existing `fullstack.py`
  and `orchestrator.py`; no replacement of baseline orchestrator semantics.

## Fullstack skill execution guardrails

- Default fullstack path remains direct Codex execution with optional skill
  overlay.
...
- `scripts/run-markdownlint.sh` — global Markdown lint wrapper using the
  repo-local `markdownlint-cli` when available.
- `scripts/check-doc-links.sh` — link validation wrapper using the repo-local
  `markdown-link-check` when available; log results in
  `timely-trackers/test-run-journal.md`.
- `tools/orchestrator/fullstack.py` — FullStack-Agent integration manager
  (config/init, upstream sync, bootstrap, planning, phase execution, status).
- `tools/orchestrator/fullstack_prompts/*.md` — phase prompt templates for
  architecture/backend/frontend/integration/back-translation runs.
- `tools/orchestrator/helpers/*.py` — Git, Codex, and CI helper shims for
  Agents SDK integrations.
- `tools/orchestrator/orchestrator.py` — entry point controlling
  Planner/Implementer/Tester/Reviewer/DevOps agents.
- `scripts/chub.sh` and `scripts/chub-mcp.sh` — repo-local Context Hub CLI/MCP
  wrappers.
- Git provider secret: add or update `OPENAI_API_KEY` so
  `.github/workflows/autofix.yml` can call Codex.
