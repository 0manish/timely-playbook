...
define a provider in the same config file and pass `--provider <name>`.
If `adapters.symphony.submit_command` is configured, Timely hands the phase to
an external Symphony service; if it is left empty, Timely falls back to local
