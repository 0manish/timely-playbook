...
3. **Review the shipped governance assets**
   ...
   - Trackers and templates:
     `.timely-playbook/local/timely-trackers/` and `.timely-core/templates/`
   - Guardrail:
     `.timely-playbook/local/AGENTS.md` plus the generated root `AGENTS.md`
   - Automation hooks:
     `.github/workflows/ci.yml`, `.github/workflows/autofix.yml`,
     `.timely-playbook/bin/check-doc-links.sh`,
     `.timely-playbook/bin/run-markdownlint.sh`
4. **Build or install the CLI**
   ...
   This refreshes the package bundle. If you prefer a global install, run
...
- **Day 4:** Conduct the first weekly sync using
  `.timely-playbook/local/timely-trackers/ceremony-agendas.md`; record
