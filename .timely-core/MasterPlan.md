...
- **Key deliverables**:
  ...
  - Governance trackers under `.timely-playbook/local/timely-trackers/`.
...
- **Key deliverables**:
  ...
  - Orchestrator commands and FullStack prompt pack under
...
- **Documentation validation**: Run
  `bash .timely-playbook/bin/run-markdownlint.sh` and
  `bash .timely-playbook/bin/check-doc-links.sh` for documentation and
  governance changes.
- **Python/unit coverage**: Run
...
- **Context Hub validation**: Run `bash .timely-playbook/bin/chub.sh validate`
  and a small search/get smoke pass when mirror scope or metadata changes.
- **Bootstrap packaging**: Run
  `bash .timely-playbook/bin/bootstrap-smoke.sh --smoke` and `make compile`
  before public template releases.
