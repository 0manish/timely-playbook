...
## Primary signals
- Documentation quality: `bash .timely-playbook/bin/run-markdownlint.sh` and
  `bash .timely-playbook/bin/check-doc-links.sh`.
- Tooling health:
  `python -m unittest discover -s .timely-core/tests -p 'test_*.py'`,
  `bash .timely-playbook/bin/chub.sh validate`, and
...
## Reporting cadence
- Log significant validation runs in
...
- Promote repeated failures into
  `.timely-playbook/local/timely-trackers/todo-backlog.md` with an owner and
  due date.
