...
- KISS-derived overlay profiles accepted in direct `codex` runs and
  `fullstack-*` orchestrator flows
- Repo-local Codex skill bundles shipped under `skills/`
...
- Section E — non-disruptive integration: keep all Codex overlays compatible with
  existing `fullstack.py` and `.orchestrator/ownership.yaml` ownership paths.

## Active skills

- `kiss-fullstack-core`: default Codex profile for standard fullstack phases.
...
- `chub-context-hub`: proper Codex skill bundle at
  `skills/chub-context-hub/SKILL.md` for repo-local `chub`/`chub-mcp` usage,
  Timely mirror validation, and public API doc retrieval through Context Hub.
- Install with `bash scripts/install-codex-skill.sh chub-context-hub`.
...
```json
{
  ...
  "repo_local_skills": {
    "chub-context-hub": {
      "path": "skills/chub-context-hub/SKILL.md",
      "description": "Repo-local Codex skill for chub wrappers, Timely mirror validation, and Context Hub MCP setup.",
      "install_command": "bash scripts/install-codex-skill.sh chub-context-hub",
  ...
  "skills": {
    ...
    "kiss-fullstack-relentless": {
      ...
      "sections": {
        ...
        "B": "Return explicit evidence for each changed file and a concise run summary in the final Codex artifact.",
